(function (window) {
  ...
  var VERSION = 'v4-gray-client-20260623e';
  ...
  var query = getQuery();
  var grayDecision = window.__SALES_CENTER_GRAY_DECISION__ || null;
  var forceStaticByGray = !!(grayDecision && grayDecision.mode === 'static' && /force-static/.test(grayDecision.reason || ''));
  var dataMode = normalizeMode(forceStaticByGray ? grayDecision.mode : (query.dataMode || query.scDataMode || (grayDecision && grayDecision.mode) || window.__SALES_CENTER_DATA_MODE__));
  ...
  function buildMeta(extra) {
    var meta = {
      ...
      grayReason: grayDecision && grayDecision.reason ? grayDecision.reason : 'none',
      ts: Date.now()
    };
    if (extra) {
  ...
  window.SalesCenterApi = {
    ...
    setMode: function (mode) {
      ...
      window.__SALES_CENTER_DATA_MODE__ = dataMode;
      grayDecision = window.__SALES_CENTER_GRAY_DECISION__ || grayDecision;
      return dataMode;
    },
    getGrayDecision: function () { return grayDecision || window.__SALES_CENTER_GRAY_DECISION__ || null; },
    call: call,
    callCloud: callCloud,
